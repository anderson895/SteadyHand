...
const LETTER_GUIDES: Record<string, [number, number][]> = {
  A: [
    // Left leg up to apex
    [108,285],[128,248],[148,208],[168,168],[188,128],[210,80],
    // Right leg down from apex
    [232,128],[252,168],[272,208],[292,248],[312,285],
    // Crossbar right → left
    [260,190],[240,190],[210,190],[180,190],[158,190]
  ],
  ...
  Q: [
    // Start at tail root (~5 o'clock on circle edge)
    [288,238],
    // CCW around full circle back to start
    [302,210],[312,178],[310,148],[298,118],[278,90],[252,68],
    [222,52],[190,48],[160,55],[135,72],[115,98],
    [102,128],[98,162],[102,195],[115,225],[135,250],
    [158,268],[185,280],[212,283],[240,278],[265,262],
    [282,242],[288,238],
    // Continue straight into tail (no jump!)
    [298,254],[310,268],[322,282],
  ],
...
const NUMBER_GUIDES: Record<string, [number, number][]> = {
  ...
  // ✅ FIXED 4
  '4': [
    // ① Left diagonal: upper-left → crossbar
    [260,40],[132,168],[108,190],
    // ② Crossbar: left → right
    [135,190],[168,190],[205,190],[240,190],[265,190],[300,190],[318,190],
    // ③ Retrace left to stem (hidden — same y=190)
    [300,190],[265,190],
    // ④ UP the right vertical to very top (makes full right side visible)
    [265,160],[265,128],[265,95],[265,62],[265,40],
    // ⑤ DOWN the full right vertical to bottom
    [265,68],[265,100],[265,135],[265,165],[265,190],[265,220],[265,250],[265,280],[265,295],
  ],
  ...
  // ✅ FIXED 9
  // Same strategy as Q: start at where tail connects (~4 o'clock on circle edge).
  // Trace full circle CCW back to start, then continue into the tail downward.
  '9': [
    // Start at tail root (~4 o'clock)
    [295,155],
    // CCW around circle
    [300,125],[295,95],[278,72],[255,55],[225,45],[195,45],
    [165,52],[140,68],[122,92],[112,122],[112,155],
    [120,185],[138,210],[162,228],[188,238],[215,238],
    [242,232],[265,215],[282,192],[295,165],[295,155],
    // Continue into tail downward (no jump!)
    [295,185],[292,218],[290,252],[288,285],
  ],
};
